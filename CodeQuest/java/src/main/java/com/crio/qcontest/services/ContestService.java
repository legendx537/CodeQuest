...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        // orElseThrow will internally conbvert the optinal to Respective types by itself
        // herte respective types are User And Contest
        User user =userRepository.findByName(userName).orElseThrow(() -> new RuntimeException("User name:"+ userName + "not found!"));
        Contest contest= contestRepository.findById(contestId).orElseThrow(() -> new RuntimeException("Contest id::"+ contestId + "not found!"));
        

        if(ContestStatus.IN_PROGRESS.equals( contest.getContestStatus()) ){
            throw new RuntimeException("Contest has already started!");
        }
        if(ContestStatus.ENDED.equals(contest.getContestStatus() ) ){
    ...
    public List<Contestant> contestHistory(Long contestId) {
        ...
        Contest contest = contestRepository.findById(contestId).orElseThrow(
            () -> new RuntimeException("Contest: "+contestId+" not found!"));
         
        if( contest.getContestStatus().equals(ContestStatus.NOT_STARTED) )
         throw new RuntimeException("History for Contest id:" + contestId + " not found!");
